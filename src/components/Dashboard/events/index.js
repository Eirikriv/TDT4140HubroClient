    ...
    if(!this.props.data.loading){
      return(
        <section className="section-block" id="setting-event">
          ...
          <article>
            <p>Now that you are logged in, we suggest that you choose which courses you want Hubro to add to your google calendar. After selecting, hit the button to add events to your calendar</p>
          </article>
          <h6>Last updatadet: {this.props.data.user.updatedCalendars}</h6>
          <button onClick={this.onClickHandler}> Add events</button>
        </section>
      )
    }
    return(
